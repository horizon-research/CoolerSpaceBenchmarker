...
def adapt(original_image: np.ndarray, original_illuminant: np.ndarray, target_illuminant: np.ndarray):
    ...
    original_image_lms = (original_image_linear @ RGB2XYZ) @ XYZ2LMS

    # Calculating factor to adjust lms cones by
    original_illuminant_lms = original_illuminant @ Light2LMS
    target_illuminant_lms = target_illuminant @ Light2LMS
    factor = (target_illuminant_lms / original_illuminant_lms).reshape((1, 3))
    factor_projected = project @ factor
    vk_matrix = factor_projected * identity

    # Modulate image
    new_image_lms = original_image_lms @ vk_matrix

    # Translate back to sRGB space
    new_image_linear = (new_image_lms @ LMS2XYZ) @ XYZ2RGB
    new_image_sRGB = (new_image_linear ** 0.45454545454545453) * 255

    return new_image_sRGB.reshape(original_shape)
